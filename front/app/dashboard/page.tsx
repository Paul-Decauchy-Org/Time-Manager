...
function ClockHistoryCard({
  timeEntries,
  loading,
  error,
}: Readonly<{ timeEntries: any[] | undefined; loading: boolean; error: any }>) {
  ...
  const rows = useMemo(() => {
    return (timeEntries ?? [])
      ...
      .sort(
        (a: any, b: any) =>
          new Date(b.arrival ?? b.day ?? 0).getTime() -
          new Date(a.arrival ?? a.day ?? 0).getTime(),
      )
      .slice(0, 10)
      .map((e: any) => {
  ...
  const formatDuration = (a?: Date | null, d?: Date | null) => {
    ...
    const minutes = Math.max(
      0,
      Math.round((d.getTime() - a.getTime()) / 60000),
    );
        ...
        {!loading && !error && rows.length > 0 && (
          ...
          >
            <TabsList className="mb-4" style={{ width: "50%" }}>
              <TabsTrigger value="table" className="py-2">
                {" "}
                Tableau{" "}
              </TabsTrigger>
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            variant={variant}
                            className="inline-flex items-center gap-2 px-3 py-1.5 text-xs md:text-sm rounded-full text-white"
                          >
                  ...
                  <div key={`${g.label}-${gi}`} className="space-y-3">
                    ...
                    <ol className="relative ms-3 border-s pl-6">
                      {g.items.map((r: any, idx: number) => {
                        ...
                        else if (r.status === "Terminé")
                          badgeVariant = "default";
                        return (
                          <li key={r.id ?? idx} className="mb-5 ms-4">
                            <span
                              className={`absolute -start-2.5 mt-2 size-3 rounded-full ring-4 ${dotColor}`}
                            />
