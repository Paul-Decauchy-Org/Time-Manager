...
export function Heatmap({
  ...
  const header = title
    ? React.createElement("div", { className: "text-sm font-semibold mb-2" }, title)
    : null

  const rowHeader = rowLabels
    ? React.createElement(
        "div",
        { className: "mr-2 grid gap-1", style: { gridTemplateRows: `repeat(${rows}, minmax(0, 1fr))` } },
        ...rowLabels.slice(0, rows).map((l, i) =>
          React.createElement(
            "div",
            { key: i, className: "text-[10px] text-muted-foreground leading-none self-center flex items-center", style: { blockSize: `${minTrackPx}px` } },
            l
          )
        )
      )
  ...
  const colHeader = colLabels
    ? React.createElement(
        "div",
        {
          ...
          style: { gridTemplateColumns: `repeat(${cols}, minmax(${minTrackPx}px, 1fr))` },
        },
        ...colLabels.slice(0, cols).map((l, i) =>
          React.createElement(
            "div",
            { key: i, className: "text-[10px] text-muted-foreground text-center" },
            l
          )
        )
      )
