...
export function useTeamMembers(teamId: string) {
    ...
    const { data: teamMembersData, loading: loadingMembers, refetch: refetchMembers } = useQuery(UsersByTeamDocument, {
        variables: { teamID: teamId },
        skip: !teamId, // Ne charge pas si pas de teamId
        fetchPolicy: 'cache-and-network',
    });

    const { data: timeEntriesData, loading: loadingTimeEntries } = useQuery(TimeTableEntriesDocument, {
        fetchPolicy: 'cache-and-network',
    });
